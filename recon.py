...
def collect_crtsh(domain: str, out_dir: Path) -> List[str]:
...
def probe_fallback_requests(subs: List[str], out_dir: Path) -> Path:
...
def detect_login_admin(alive_urls_file: Path, out_dir: Path) -> List[str]:
    hits: List[str] = []
    ...
    for url in urls:
        try:
            ...
            if r.status_code >= 600:
                continue
            marker = classify_page(r.url, r.text[:200000])
            if marker:
                hits.append(f"{r.url} ({marker})")
        except Exception:
            continue
...
def run_eyewitness(alive_urls_file: Path, out_dir: Path) -> Path:
    ew_root = Path.home() / "EyeWitness"
    ew_py = ew_root / "Python" / "EyeWitness.py"
    venv_py = ew_root / "venv" / "bin" / "python"
    ...
    if not ew_py.exists():
        ...
        msg = f"[!] EyeWitness not found at {ew_py} -> skipping"
        if hint:
            msg += f" (install: {hint})"
        print_bad(msg)
        return out_path

    python_bin = str(venv_py) if venv_py.exists() else (which("python3") or "python3")
...
def main() -> int:
    ...
    if not args.no_crtsh:
        subs += collect_crtsh(domain, out_dir)
    ...
    if not args.no_httpx and which("httpx"):
        ...
        if not alive_urls_file.exists() or not read_lines(alive_urls_file):
            alive_urls_file = probe_fallback_requests(subs, out_dir)
    else:
        alive_urls_file = probe_fallback_requests(subs, out_dir)
    ...
    login_hits = detect_login_admin(alive_urls_file, out_dir)
    if login_hits:
    ...
    if not args.no_eyewitness:
        eye_out = run_eyewitness(alive_urls_file, out_dir)
        if eye_out.exists():
